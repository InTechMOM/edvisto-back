...
import { SchemaAssignment } from '../../validators/assignment.js';
...
async function assignment(request, response, next) {
  try {
    ...
    const teacher = await UserEV.findOne({
      email: emailTeacher,
      rol: 'Soy Docente',
    });
    ...
    if ((startDate && !finishDate) || (!startDate && !finishDate)) {
      return response
        .status(422)
        .json('Both start and finish dates are required or none at all.');
    }
    ...
    if (
      ...
      return response
        .status(422)
        .json(
          'Invalid date. The start/end date must be equal to or later than the current date.'
        );
    } else {
      next(error);
    }
