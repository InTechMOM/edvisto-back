...
import authorizationRol from '../../middlewares/authorizationRol.js';
...
const login = async (request, response, next) => {
  try {
      ...
      if (error.code === 'auth/wrong-password') {
        ...
        if (user.attemptsFailed === attemptsAllowed) {
          ...
          return response.status(403).json({
            error: 'Blocked account. A password reset email has been sent',
          });
        }
    ...
    if (error.message === 'auth/too-many-requests') {
      return response
        .status(403)
        .json('Blocked account. A password reset email has been sent');
    }
    next(error);
  }
};
