  ...
  try {
    ...
    if (!folder) {
      return response.status(404).json({
        error: 'No se encontró la carpeta especificada para este usuario',
      });
    }
    ...
    if (!folder.email || folder.email !== email) {
      return response.status(403).json({
        error: 'El usuario no está autorizado para modificar esta carpeta',
      });
    }
    ...
    response.status(200).json({
      message: 'Link agregado correctamente a la carpeta',
      data: folder,
    });
  } catch (error) {
    console.error(error);
    response
      .status(500)
      .json({ error: 'Error al agregar el link a la carpeta' });
  }
};
