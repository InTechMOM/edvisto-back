...
import { schemaFeedback } from '../../validators/feedback.js';
...
async function studentFeedback(request, response, next) {
  try {
    ...
    if (!value.feedback?.skills) {
      return response
        .status(400)
        .json({ error: 'Qualification must not be empty' });
    }
    ...
    const feedback = await StudentsFeedback.findByIdAndUpdate(
      id,
      { feedback: request.body.feedback, qualified: true },
      { new: true }
    );
    if (!feedback) {
