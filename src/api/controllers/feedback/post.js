...
import { schemaDelivery } from '../../validators/feedback.js';
...
async function delivery(request, response, next) {
  ...
  try {
    ...
    const student = await UserEV.findOne({
      email: emailStudent,
      rol: 'Soy Estudiante',
    });
    if (!student) {
    ...
    if (!('finishDate' in isAssigned && 'startDate' in isAssigned)) {
      if (
        isAssigned.startDate !== undefined &&
        isAssigned.finishDate !== undefined
      ) {
    ...
    if (error.code === mongoDBEmailDuplicate) {
      return response
        .status(409)
        .json({ error: 'URL is assigned to another user' });
    }
    next(error);
  }
}
